...
class Model(object):
	...
	def __init__(self, config):
		...
		with tf.name_scope('att'):
			self.r = tf.reshape(self.tail - self.head, [relation_size, -1]) * self.input_r_n + self.NAattention * (1 - self.input_r_n)
...
if __name__ == "__main__":
	...
	config.tripleTotal = lib_kg.getTripleTotal() 
	...
	log = open("./data/FB60K/init_vec_E.txt", "r")
	...
	with tf.Graph().as_default():
		...
		with sess.as_default():
			...
			def outEmbedding(str1):
				word_embeddings, relation_embeddings, position_embeddings, relation_attention, attention, W, B, transfer_w, NAattention = sess.run([m.word_embeddings, m.relation_embeddings, m.position_embeddings, m.relation_attention, m.attention, m.W, m.b, m.transfer_w, m.NAattention])
